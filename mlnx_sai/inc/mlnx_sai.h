...
sai_status_t mlnx_acl_port_event_handle(_In_ const mlnx_port_config_t *port, _In_ sai_port_event_t event);
...
#define ACL_MAX_PORT_LIST_NUMBER   100
...
typedef struct _acl_entry_ports_ref_t {
...
typedef struct _acl_entry_db_t{
    ...
    uint32_t               prev;
    bool                   is_used;
    acl_entry_ports_refs_t ports_refs;
} acl_entry_db_t;

typedef struct _acl_ports_references_t {
...
typedef struct _acl_setting_tbl_t {
    bool           bg_stop;
    bool           initialized;
    bool           ingress_direction_bound;
    bool           egress_direction_bound;
    acl_port_ref_t ports_used[MAX_PORTS];
    cl_plock_t     lock;
#ifndef _WIN32
...
static inline mlnx_port_config_t *mlnx_port_config_get(uint32_t idx)
{
    if (idx >= MAX_PORTS) {
        return NULL;
    }

    return &mlnx_ports_db[idx];
}
