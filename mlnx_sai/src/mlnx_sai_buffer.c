...
sai_status_t mlnx_sai_configure_multicast_buffers_defaults()
{
    ...
    for (ind = 0; ind < count; ind++) {
        sx_port_reserved_buff_attr_arr[ind].attr.multicast_buff_attr.size = SAI_SDK_DEFAULT_MC_BUFFER_RESERVED_SIZE;
        sx_port_shared_buff_attr_arr[ind].attr.multicast_shared_buff_attr.max.max.alpha = SX_COS_PORT_BUFF_ALPHA_INFINITY_E;
    }
    if (SAI_STATUS_SUCCESS !=
        (sai_status = mlnx_sai_buffer_configure_reserved_buffers(mc_port_logical, sx_port_reserved_buff_attr_arr, count))) {
    }
    if (SAI_STATUS_SUCCESS !=
        (sai_status = mlnx_sai_buffer_configure_shared_buffers(mc_port_logical, sx_port_shared_buff_attr_arr, count))) {
        goto cleanup;
    }
cleanup:
    if (sx_port_shared_buff_attr_arr) {
